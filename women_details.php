...
<head>
    ...
    <title>ETERNA - WOMEN DETAILS</title>
...
<body>
  ...
  <?php
    ...
    $women_res = select("SELECT * FROM `women` WHERE `id`=? AND `status`=? AND `removed`=?",[$data['id'],1,0],'iii');

    if (mysqli_num_rows($women_res) == 0) {
      redirect('women.php');
    }

    $women_data = mysqli_fetch_assoc($women_res);
  ...
  <div class="container">
    <div class="row">
      <div class="col-12 my-4 mb-4 px-4">
        <h2 class="fw-bold h-font" style="color: #c23737;"><?php echo $women_data['brand'] ?></h2>
        ...
        <div id="menCarousel" class="carousel slide carousel-fade">
          <div class="carousel-inner">
            <?php 
              ...
              $women_img = WOMEN_IMG_PATH."thumbnail.jpg";
              $img_q = mysqli_query($con,"SELECT * FROM `women_images` 
                WHERE `women_id`='$women_data[id]'");
              ...
              if (mysqli_num_rows($img_q) > 0) {
                ...
                while ($img_res = mysqli_fetch_assoc($img_q)) {
                  echo"
                    <div class='carousel-item $active_class'>
                      <img src='".WOMEN_IMG_PATH.$img_res['image']."' class='d-block w-100 rounded'>
                    </div>";
                    $active_class='';
                }
                ...
                echo "<div class='carousel-item active'>
                  <img src='$women_img' class='d-block w-100'>
                </div>";
              }
        ...
        <div class="card border-0 shadow-sm rounded-3">
          <div class="card-body me-0">
            <?php 
              ...
              // Product name
              echo<<<name
                <h6 class="mb-2" style="font-size: 23px;">$women_data[name]</h6>
              name;

              // Product category
              echo<<<cat
                <p class="text-secondary mb-2" style="font-size: 0.8rem;">$women_data[category]</p>
              cat;

              // check quantity and show message according to it 
              $stock_status = ($women_data['quantity'] <= 0) ? "<span class='text-danger ms-3 fw-bold' style='font-size: 0.8rem;'>*Out of Stock</span>" : "";
              
              // Product price
              echo<<<price
                <p class="fw-bold mb-2" style="font-size: 1.2rem;">₹$women_data[price]/- $stock_status</p>
              price;
              ...
              if (!$settings_r['shutdown']) {
                ...
                if ($women_data['quantity'] > 0) {
                    // Product is in stock, show the normal button
                    $book_btn = "<button onclick='checkLoginToBook($login, $women_data[id])' class='btn btn-sm btn-outline-dark w-100 shadow-none mb-2 mb-lg-0'> Buy Now </button>";
                } else {
                ...
                <div class="accordion" id="accordionExample">
                  <div class="accordion-item">
                      ...
                      <div class="accordion-body">
                          <p class="mb-1" style="font-size: 0.8rem;">$women_data[description]</p>
                      </div>
                      </div>
                  </div>
                  <div class="accordion-item">
                      ...
                      <div class="accordion-body">
                          ...
                          <p class="text-secondary mb-3" style="font-size: 0.8rem;">$women_data[brand]</p>
                          <p class="mb-0" style="font-size: 0.8rem;">Model Number </p>
                          <p class="text-secondary mb-3" style="font-size: 0.8rem;">$women_data[modelnumber]</p>
                          <p class="mb-0" style="font-size: 0.8rem;">Movement </p>
                          <p class="text-secondary mb-3" style="font-size: 0.8rem;">$women_data[movement]</p>
                          <p class="mb-0" style="font-size: 0.8rem;">Case Material </p>
                          <p class="text-secondary mb-3" style="font-size: 0.8rem;">$women_data[casematerial]</p>
                          <p class="mb-0" style="font-size: 0.8rem;">Strap Material </p>
                          <p class="text-secondary mb-3" style="font-size: 0.8rem;">$women_data[strapmaterial]</p>
                          <p class="mb-0" style="font-size: 0.8rem;">Dial Size </p>
                          <p class="text-secondary mb-3" style="font-size: 0.8rem;">$women_data[dialsize]</p>
                          <p class="mb-0" style="font-size: 0.8rem;">Water Resistance </p>
                          <p class="text-secondary mb-3" style="font-size: 0.8rem;">$women_data[waterresistance]</p>
                          <p class="mb-0" style="font-size: 0.8rem;">Warranty </p>
                          <p class="text-secondary mb-3" style="font-size: 0.8rem;">$women_data[warranty]</p>
                      </div>
                      </div>
                  </div>
                </div>
